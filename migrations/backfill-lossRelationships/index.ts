...
import { lossRelationshipSlugs } from "./lossRelationshipSlugs";
...
const slugInFilter = lossRelationshipSlugs.map((s) => `"${s}"`).join(",");

export default defineMigration({
  title:
    "Backfill lossRelationship featuredResources / coverImageRef / description / shortDescription from matching category",
  documentTypes: ["lossRelationship"],
    ...
    async document(doc, context) {
      ...
      if (!cat) {
        if (!warnedMissingCategory.has(slug)) {
          ...
          console.warn(
            `[backfill-lossRelationships] no category found for slug "${slug}"`
          );
        }
        return [];
      }
