...
| `audiences`     | array of references → `audience` | Dereferenced to IDs. Defaults to `[]`              |
...
**Known audience slugs**: `latino-and-hispanic-americans`, `african-american-black`, `asian-american-and-pacific-islander`, `people-with-disabilities`, `lgbtq-community`, `indigenous-communities`
...
| `resourcePageLink`       | Link to resource listing      | `label`, `category`, `resourceTypes`, `audience` |
...
| `/{categorySlug}/{audienceSlug}` | `category` × `audience` intersection               | Resources filtered by both category tree AND audience                               |
| `/{audienceSlug}`                | `audience` + pruned category trees                 | Category navigation filtered to show only branches with resources for that audience |
| `/topics/{audienceSlug}`         | Root category × audience                           | Filtered subcategory view                                                           |
...
- Category × Audience pages are **only generated when resources exist** for that combination (no empty pages)
- Audience landing pages **prune the full category tree** to remove branches with no resources for that audience
- A `resourceExistenceSet` (Set of `categoryId:audienceId` strings) enables O(1) lookup during tree pruning
...
2. **Audiences ↔ Resources**: Many-to-many. Tagging enables audience-filtered views. Pages are only generated when resources exist for a combination.
...
1. **The `categories` and `audiences` arrays on resources are the primary content organization mechanism.** If a resource isn't tagged, it won't appear anywhere except potentially as a featured resource.
...
3. **Audience tagging drives page generation.** Empty category×audience intersections produce no page — so tagging resources with appropriate audiences expands the site's reach.
...
8. **Slug fields generate URLs.** Category and audience slugs directly become URL segments. Changing a slug would change the URL (and break existing links).
